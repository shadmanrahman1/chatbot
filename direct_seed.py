...
def seed_database_directly():
    """Seed the database directly using Railway connection"""
    # Railway DATABASE_URL format: mysql://user:password@host:port/database
    database_url = "mysql://root:bOFivGQlgWqIqJsopJroXStxJzmjmiPI@roundhouse.proxy.rlwy.net:58662/railway"

    try:
        config = parse_database_url(database_url)
        print(f"🔗 Connecting to Railway MySQL at {config['host']}:{config['port']}")
...
if __name__ == "__main__":
    print("🌱 Direct Railway Database Seeding")
    print("=" * 40)

    success = seed_database_directly()

    if success:
        print("\n🎉 Database seeding completed!")
        print(
            "💡 You can now test with: https://web-production-e9648.up.railway.app/health"
        )
    else:
        print("\n❌ Seeding failed!")
        print("💡 Check your Railway MySQL connection details")
